...
export default function Step1_Scope({ value, onChange, onSave }) {
  const { th } = useTheme()
  ...
  const addStakeholder = () => {
    ...
    onChange({ stakeholders: [...value.stakeholders, { ...newSH, id: Date.now() }] })
    setNewSH({ name: '', role: '' })
  }
  const removeStakeholder = (id) =>
    onChange({ stakeholders: value.stakeholders.filter((s) => s.id !== id) })
      ...
      <Card className="overflow-hidden">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {activeTab === 'context' && (
              <>
                <TextField label="Nombre del Proyecto" value={value.projectName}
                  onChange={(v) => onChange({ projectName: v })}
                  placeholder="Ej. Programa de becas universitarias XIGNUX 2026" />
                <TextArea label="Descripción / Contexto de la medición" value={value.description}
                  onChange={(v) => onChange({ description: v })} rows={4}
                  placeholder="Describa el proyecto, su alcance, la problemática que aborda y el contexto de la medición SROI..." />
                <DateField label="Fecha de inicio del Proyecto" value={value.startDate}
                ...
                {SCOPE_QUESTIONS.map(({ key, label, placeholder }, i) => (
                  <div key={key} className="flex gap-3">
                    ...
                    <div className="flex-1">
                      <TextArea label={label} value={value.scopeAnswers[key]}
                        onChange={(v) => onChange({ scopeAnswers: { ...value.scopeAnswers, [key]: v } })}
